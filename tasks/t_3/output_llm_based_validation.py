...
def validate(llm_output: str, llm_client: AzureChatOpenAI) :
    ...
    return (prompt | llm_client | parser).invoke({"user_input": llm_output})
...
def main(soft_response: bool = False):
    ...
    while True:
        ...
        validation = validate(ai_message.content, llm_client)

        if validation.valid:
            messages.append(ai_message)
            print(f"🤖Response:\n{ai_message.content}")
        elif soft_response:
            filtered_ai_message = llm_client.invoke(
                [
